...
function Theater({item}:{item:typeof program[number]}){
 ...
 const [reload,setReload]=useState(0),[single,setSingle]=useState(false);
 ...
 onStateChange:(e:Event)=>{if(!active)return;if(i===0&&e.data===1)e.target.mute();update(setStates,i,label(e.data));},
 ...
 function restart(){command((p,i)=>{if(i===0&&(single||!item.cartoonList))p.seekTo(0,true);else p.playVideoAt(0);if(i===0)p.mute();else p.unMute();p.playVideo();});setMessage("Restart requested from the opening episode and first album track.");}
 return <>
 ...
 <div className="picture frame-holder" key={"picture-"+reload+"-"+single}>{origin&&<iframe ref={el=>{frames.current[0]=el;}} src={embedURL(single?undefined:item.cartoonList,item.video,origin,true)} title={item.title+" cartoon player"} allow="autoplay; encrypted-media; fullscreen; picture-in-picture" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/>}</div>
 ...
 <div className="music frame-holder" key={"music-"+reload+"-"+single}>{origin&&<iframe ref={el=>{frames.current[1]=el;}} src={embedURL(item.albumList,undefined,origin,false)} title={item.artist+" "+item.album+" full album player"} allow="autoplay; encrypted-media; fullscreen; picture-in-picture" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/>}</div>
 <div className="deck-foot"><Button className="small-button" variant="outline" disabled={!ready[1]} onClick={()=>{try{players.current[1]?.nextVideo();}catch{setMessage("Use the album player's next button.");}}}>Next song →</Button><a href={"https://www.youtube.com/playlist?list="+item.albumList} target="_blank" rel="noreferrer">Album source ↗</a></div>
 ...
 <Button className="play-button" disabled={!ready.every(Boolean)} onClick={start}>▶ Start both</Button>
 ...
 {item.cartoonList&&<Button variant="ghost" onClick={()=>{command(p=>p.pauseVideo());setSingle(v=>!v);setMessage("Source changed. Press Start both when ready.");}}>{single?"Restore episode playlist":"Try opening episode only"}</Button>}
 </div></section></>;
}
